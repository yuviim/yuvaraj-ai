...
const pillarLabels: Record<string, string> = {
  DP: "Data Platforms", ES: "Enterprise AI", AA: "Agentic AI", SV: "Sovereign AI", KX: "Know Exasol",
};
...
export default function ContentStudio() {
  ...
  const handleSave = async (publishStatus: string) => {
    ...
    try {
      ...
      const res = await fetch(endpoint, {
        ...
        body: JSON.stringify({ ...content, status: publishStatus }),
      });
      if (res.ok) {
        if (typeof window !== "undefined") window.alert(publishStatus === "PUBLISHED" ? "Published!" : "Saved as draft!");
        setContent({ title: "", body: "", pillar: "DP", format: "ARTICLE", status: "DRAFT" });
        setEditingId(null);
        fetchLibrary();
      } else {
  ...
  const handleEdit = (item: ContentItem) => {
    setContent({ title: item.title, body: item.body || "", pillar: item.pillar, format: item.format, status: item.status });
