...
export async function GET(request: NextRequest) {
  try {
    ...
    // When fetching videos, also include items that contain a YouTube URL in body
    // (common when videos are added as ARTICLE format with embedded YouTube links)
    const formatFilter = format === "VIDEO"
      ? {
          OR: [
            { format: "VIDEO" },
            {
    ...
    const items = await prisma.contentItem.findMany({
      where: {
        ...(pillar && { pillar: pillar as string }),
        ...formatFilter,
